...
int main ( int argc, char *argv[] ){ 
  ...
  timestamp();
  node_filename = "../datos/node.dat";
  triangle_filename = "../datos/trian.dat";
...
int diaedg ( double x0, double y0, double x1, double y1, double x2, double y2, 
  ...
  if ( tola < ca && tolb < cb )	value = -1;
  else if ( ca < -tola && cb < -tolb )	value = 1;
  else{
...
int dtris2 ( int point_num, int base, double point_xy[], int *tri_num, 
  ...
  *tri_num = j - 2;
  ...
  for ( i = j+1; i <= point_num; i++ ){
    ...
    if ( ledg <= 2 )	m2 = tri_vert[3*(ltri-1)+ledg];
    else 	m2 = tri_vert[3*(ltri-1)+0];
    ...
    for ( ; ; ){
      ...
      if ( e <= 2 )	m1 = tri_vert[3*(t-1)+e];
      else 	m1 = tri_vert[3*(t-1)+0];
...
# define INCX 5
  ...
  for ( i2lo = i4_max ( ilo, 1 ); i2lo <= i4_min ( ihi, m ); i2lo = i2lo + INCX ){
    ...
    inc = i2hi + 1 - i2lo;
    //for ( i = i2lo; i <= i2hi; i++ )  cout << setw(7) << i << "       ";
...
//
  ...
  for ( ; ; ){
    ...
    if ( e <= 2 ) b = tri_vert[3*(t-1)+e];
    else  b = tri_vert[3*(t-1)+0];
  ...
  for ( ; ; ){
    ...
    while ( 0 < tri_nabe[3*(t-1)+e-1] ){
      ...
      if ( tri_vert[3*(t-1)+0] == b ) e = 3;
      else if ( tri_vert[3*(t-1)+1] == b )  e = 1;
      else  e = 2;
